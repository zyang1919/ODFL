...
## read data 
Yall_50 <- get(load(paste(dir_sinuous, "Sinuous Simulated 200 Response (Y) n=50.RData", sep = '/')))
Uall_50 <- get(load(paste(dir_sinuous, "Sinuous Simulated 200 Response (Uall) n=50.RData", sep = '/')))
Zall_50 <- get(load(paste(dir_sinuous, "Sinuous Simulated 200 Response (Zall) n=50.RData", sep = '/')))
Xmall_50 <- get(load(paste(dir_sinuous, "Sinuous Simulated 200 Response (Xm) n=50.RData", sep = '/')))
...
for(s in 1:nsim){
  ...
  for (k in 1:aiter) {
    ...
    for(i in 1:n){
      for(j in 1:(mi-1)){
        RSSvi <- (Vt[j,i] - gamma0t - gamma1t*Wt[i] - gamma2t*Y[j+1,i])^2
      }
      RSSv <- RSSv + RSSvi
    }
    Azstar <- (n*(mi-1))/2 + Az
  ...
  ft_i <- 5*sin(pi*(X_design-10)/10)/(1+2*((X_design-10)/5)^2*(sign((X_design-10)/5)+1))+4
  fhat <- f_sim[[s]]
...
Rst.50sz.NB.BNP.Sinuous.200sim <- list()
Rst.50sz.NB.BNP.Sinuous.200sim[[1]] <- eta_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[2]] <- eta_sd
Rst.50sz.NB.BNP.Sinuous.200sim[[3]] <- gamma1_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[4]] <- gamma1_sd_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[5]] <- gamma2_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[6]] <- gamma2_sd_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[7]] <- rho_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[8]] <- rho_sd_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[9]] <- lpml_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[10]] <- f_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[11]] <- f_sd_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[12]] <- W_sim
Rst.50sz.NB.BNP.Sinuous.200sim[[13]] <- eta_mse
Rst.50sz.NB.BNP.Sinuous.200sim[[14]] <- imse_sim
names(Rst.50sz.NB.BNP.Sinuous.200sim) <- c("eta", "eta_sd", "gamma1", "gamma1_sd", 
                                           "gamma2", "gamma2_sd",  
                                           "rho", "rho_sd", 
                                           "lpml_sim", "f_sim", "f_sd", 
                                           "W_sim", "eta mse", "imse sim")
dir_rst <- "~/Desktop/Dissertation/ODFL/Simulation Revision/Result Sinuous"
save(Rst.50sz.NB.BNP.Sinuous.200sim, file = paste(dir_rst, "Rst.50sz.NB.BNP.Sinuous.200sim.RData", sep = '/'))
