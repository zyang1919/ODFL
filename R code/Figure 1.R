...
## result
rst <- get(load(paste(dir_rst, "Rst.50sz.NB.BP.Sinuous.200sim.RData", sep = '/')))
rst.bnp <- get(load(paste(dir_rst, "Rst.50sz.NB.BNP.Sinuous.200sim.RData", sep = '/')))
...
n <- ncol(Y)
theta.bp <- rst$theta_sim[[2]]
theta.bp <- matrix(theta.bp, nrow = 4)
eta.bp <- rst$eta[2]
W.bp <- rst$W_sim[[2]]
W.bp <- mean(W.bp)
yhat <- theta.bp[1,] + theta.bp[2,]*Xm + theta.bp[3,]*(Xm)^2 + + theta.bp[4,]*(Xm)^3 + eta.bp*log(W.bp)
...
cols <- rainbow(3, s=0.5)
lines(Xm, yhat, col = cols[1], lwd=4)

f.bnp <- rst.bnp$f_sim[[2]]
f.bnp <- matrix(f.bnp, 19)
eta.bnp <- rst.bnp$eta[2]
W.bnp <- rst.bnp$W_sim[[2]]
W.bnp <- mean(W.bnp)
yhat.bnp <- f.bnp + eta.bnp*log(W.bnp)
lines(Xm, yhat.bnp, col = cols[2], lwd=4)
...
### n=100
## data set
Yall_100 <- get(load(paste(dir_sinuous, "Sinuous Simulated 200 Response (Y) n=100.RData", sep = '/')))
Uall_100 <- get(load(paste(dir_sinuous, "Sinuous Simulated 200 Response (Uall) n=100.RData", sep = '/')))
Zall_100 <- get(load(paste(dir_sinuous, "Sinuous Simulated 200 Response (Zall) n=100.RData", sep = '/')))
Xmall_100 <- get(load(paste(dir_sinuous, "Sinuous Simulated 200 Response (Xm) n=100.RData", sep = '/')))

## result
rst <- get(load(paste(dir_rst, "Rst.100sz.NB.BP.Sinuous.200sim.RData", sep = '/')))
rst.bnp <- get(load(paste(dir_rst, "Rst.100sz.NB.BNP.Sinuous.200sim.RData", sep = '/')))
