...
Rst.tkn.PH.BNP <- list()
Rst.tkn.PH.BNP[[1]] <- W_bnp
Rst.tkn.PH.BNP[[2]] <- eta_bnp
Rst.tkn.PH.BNP[[3]] <- sd(etatt)
Rst.tkn.PH.BNP[[4]] <- gamma
Rst.tkn.PH.BNP[[5]] <- sd(gammatt)
Rst.tkn.PH.BNP[[6]] <- mean(rhott)
Rst.tkn.PH.BNP[[7]] <- sd(rhott)
Rst.tkn.PH.BNP[[8]] <- f_bnp
Rst.tkn.PH.BNP[[9]] <- apply(bettt, 2, mean)
Rst.tkn.PH.BNP[[10]] <- apply(bettt, 2, sd)
Rst.tkn.PH.BNP[[11]] <- lpml
names(Rst.tkn.PH.BNP) <- c("mean of W bnp", "eta hat", "eta sd", 
                           "gamma hat", "sd gamma", "rho hat", 
                           "sd rho", "f hat", 
                           "beta hat", "beta sd", "lpml")

save(Rst.tkn.PH.BNP, file = paste(dir_tkn, "Rst.tkn.PH.BNP.RData", sep = '/'))
